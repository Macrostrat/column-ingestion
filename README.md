...
All tables can include a `comments` and/or `notes` field for additional notes or clarifications.
...
multiple columns. This feature requires a separate table of surfaces which
carry chronostratigraphic information and can be linked across multiple columns.
...
within the project. __Unlike other sheets, this sheet is laid out as key-value pairs, with one field per row.__
